...
export default async function ProfessionsPage() {
    ...
    const professions = data.reduce((acc, { member_name: memberName, profession_name: professionName, spell_count, realm_slug: realmSlug }) => {
    ...
    }, [] as { memberName: string, realmSlug: string, professions: { name: string, icon: string, recipeCount: number }[] }[])
    ...
    return (
        <main className="flex w-full h-full flex-col">
            ...
            {(professions).map((member) => {
                const { memberName, realmSlug, professions } = member
                return <Link
                    href={`${createRosterMemberRoute(memberName, realmSlug)}/professions`}
                    key={memberName}
                    ...
                    <div className="flex gap-3 justify-center items-center md:w-[186px]">
                        <CharacterAvatar realm={realmSlug || GUILD_REALM_SLUG}
