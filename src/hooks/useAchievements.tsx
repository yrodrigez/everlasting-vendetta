...
import { applyReducer } from "@/lib/achievements";
    ...
    supabase.rpc('insert_login_if_not_exists', { _member_id: characterId })
        .then(({ data, error }) => {
            if (error) {
                ...
                //console.log('Inserted login:', data);
            }
        })
}
...
async function recentlyLoggedIn(supabase: SupabaseClient, characterId: number) {
    const { data, error } = await supabase
        ...
        .filter('created_at', 'gte', new Date(Date.now() - 1000 * 60 * 60).toISOString()) // 1 hour
        .limit(1)
    ...
    if (!data?.length) {
        saveLoginDate(supabase, characterId)
        return false
    }
...
export default function useAchievements() {
    ...
    const { data: achievements, error, isLoading: loadingAchievements } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (hasRecentLog) {
                console.log('Not fetching achievements, recently logged in')
                return {}
            }
            ...
            const {
                ...
                .overrideTypes<MemberAchievement[], { merge: false }>()
