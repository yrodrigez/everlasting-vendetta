create or replace function achievement_raid_rookie(character_name text)
    returns TABLE
            (
                achieved boolean,
                progress numeric
...
BEGIN
    RETURN QUERY
        ...
        SELECT CASE
                   WHEN COALESCE(rc.count, 0) >= 5 THEN TRUE
                   ELSE FALSE
                   END                              AS has_two_alts_raided,
               CASE
                   WHEN COALESCE(rc.count, 0) >= 5 THEN 100.0
                   ELSE COALESCE(rc.count, 0.0) * 20.0 END AS progres
